...
Usage:
  ...
  python data/experiments/exp003b_real_data_validation.py ~/dsm-agentic-ai-data-science-methodology/

  # Phase B: score labeled CSV
  python data/experiments/exp003b_real_data_validation.py --score data/experiments/exp003b_results.csv
"""
...
def phase_a(repo_path: Path, output_csv: Path) -> None:
    ...
    print("Then run: python data/experiments/exp003b_real_data_validation.py --score <csv>")
...
def phase_b(csv_path: Path) -> None:
    ...
    print(f"Total rows: {len(rows)}")
    print(f"Labeled: {len(labeled)}, Skipped: {len(skipped)}, Insufficient: {len(insufficient)}, Unlabeled: {len(unlabeled)}")
...
def main():
    ...
    if sys.argv[1] == "--score":
        ...
        if not repo_path.is_dir():
            ...
            sys.exit(1)
        output_csv = Path("data/experiments/exp003b_results.csv")
