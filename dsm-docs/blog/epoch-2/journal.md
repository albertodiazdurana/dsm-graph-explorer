...
1. **Backlog cleanup** — Organized `dsm-docs/backlog/` with a `done/` subfolder. Moved 4 resolved items:
...
3. **Phase 5.3: User Guides** — Two new documents in `dsm-docs/guides/`:
...
4. **DSM Feedback** — Three new methodology entries and four backlog proposals:
   ...
   - Entry 20 / Proposal #17: `dsm-docs/guides/` subfolder for user-facing documentation
...
6. **README update** — Added Sprint 5 features, updated project structure (`.github/`, `scripts/`, `dsm-docs/guides/`, `dsm-docs/backlog/`), updated status line and footer.

### Aha Moments

1. **Spoke-repo config pattern** — When a project references DSM sections defined in another repository, those references will always appear as broken. The solution is a config pattern: set `dsm-docs/**/*.md` to INFO severity. The findings stay visible (useful for reference) but don't block CI. This is the first reusable configuration pattern to emerge from the project.

2. **dsm-docs/guides/ as a category** — DSM's `dsm-docs/` subfolders are all project-management artifacts (checkpoints, decisions, feedback). User-facing documentation (guides, references, how-tos) didn't have a home. Creating `dsm-docs/guides/` immediately clarified the structure. This became Entry 20, a concrete gap in DSM 4.0 Section 2.
...
6. **Session 15 Housekeeping** — Migrated `dsm-docs/inbox/` → `_inbox/` (DSM v1.3.52 compliance), `experiments/` → `data/experiments/` (Proposal #23 accepted), retroactively stamped all 50 feedback entries as Pushed, analyzed a research paper on contrastive decoding (informational, pushed to DSM Central).
