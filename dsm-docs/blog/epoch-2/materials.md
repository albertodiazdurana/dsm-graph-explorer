...
**Key insight:** The spoke-repo config pattern. When a project references DSM sections defined in another repository, those references always appear broken. The solution: set `dsm-docs/**/*.md` to INFO severity. Findings stay visible but don't block CI. First reusable configuration pattern to emerge.
