...
- `.dsm-graph-explorer.yml` — Self-validation config for this repository. Demonstrates the spoke-repo pattern: `dsm-docs/**/*.md` set to INFO severity because cross-repo references to DSM central sections are expected to be unresolved.
...
- `dsm-docs/guides/remediation-guide.md` — How to read validation output, fix each error type, understand cross-repo references, manage exclusions and severity, CI/CD integration patterns.
- `dsm-docs/guides/config-reference.md` — Complete reference for `.dsm-graph-explorer.yml`: all 4 fields with types, defaults, validation rules, pattern matching behavior, CLI interaction, hub vs spoke examples.
...
| Methodology Entry 20 | Proposal #17 | dsm-docs/guides/ subfolder for user-facing docs |
...
- [x] Remediation guide (`dsm-docs/guides/remediation-guide.md`)
- [x] Config reference (`dsm-docs/guides/config-reference.md`)
...
- [x] Blog journal entry written (`dsm-docs/blog/epoch-2/journal.md`)
...
1. **Sprint 6:** Semantic Validation — TF-IDF keyword similarity, drift detection
   - Research already done (see `dsm-docs/research/e2_handoff_graph_explorer_research.md`)
