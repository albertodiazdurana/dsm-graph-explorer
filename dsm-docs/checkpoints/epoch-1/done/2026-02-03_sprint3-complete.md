...
See `dsm-docs/decisions/DEC-002_cli_design_choices.md` for full rationale.
...
- **dsm-docs/checkpoints/:** ~80+ errors — checkpoints reference sections that existed at checkpoint time but were later renumbered
...
1. **Pre-generation brief requires explicit approval** — Documented in `dsm-docs/feedback-to-dsm/backlogs.md` as observation #6. The protocol says "explain before generating" but doesn't specify the agent must receive explicit acknowledgment before proceeding. Proposed fix: protocol must explicitly require "(1) Explain, (2) Wait for approval, (3) Generate."
...
2. Read `dsm-docs/plan/SPRINT_PLAN.md` (Sprint 4 section)
