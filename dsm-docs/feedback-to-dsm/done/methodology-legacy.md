...
- Project structure: `src/`, `tests/`, `dsm-docs/`, `outputs/`
...
|-------------|--------|------------|-----------|-----------|
| DSM 4.0 Section 2 (Project Structure) | Phase 0, S1, S3 | 4 | 3.3 | dsm-docs/ folder structure unclear for agents |
...
- **Finding:** Clear distinction between DSM 1.0 vs DSM 4.0 patterns; easy to follow. In-repo `dsm-docs/` structure worked well.
...
- **Context:** Feedback files initially created at `dsm-docs/dsm-feedback-*.md` per handoff instructions
- **Finding:** DSM specifies loose files but all other document types use subfolders (`dsm-docs/handoffs/`, `dsm-docs/decisions/`, etc.). Had to manually move files to `dsm-docs/feedback-to-dsm/` mid-project.
- **Scores:** Clarity 3, Applicability 4, Completeness 3, Efficiency 2 (Avg: 3.0)
- **Recommendation:** Standardize on `dsm-docs/feedback-to-dsm/` subfolder. See `backlogs.md`.
...
### Entry 10: dsm-docs/ Folder Structure Confusion — Agent Required Multiple Corrections
- **Date:** 2026-02-03 | **Sprint:** S3 | **Type:** Gap (Significant)
- **Context:** Sprint 3 closure required organizing documentation files. Agent needed to understand where feedback files go, where blog files go, what each dsm-docs/ subfolder contains, and the expected format of each file type.
- **Finding:** The agent demonstrated significant confusion about the dsm-docs/ folder structure despite DSM 4.0 Section 2 documentation. Specific errors requiring user correction:
  1. Attempted to create DSM backlog items directly in DSM repository instead of in project's `dsm-docs/feedback-to-dsm/backlogs.md`
  2. Confused about whether `blog.md` belongs in `dsm-docs/feedback-to-dsm/` or `dsm-docs/blog/`
  3. Did not understand the purpose distinction between `dsm-docs/backlog/` (alignment reports) vs `dsm-docs/feedback-to-dsm/` (DSM feedback)
  4. Required user to provide sql-agent `docs-folder-reference-sql-agent.md` as explicit reference
  5. Required user to point to sql-agent `dsm-docs/feedback-to-dsm/backlogs.md` and `dsm-docs/feedback-to-dsm/methodology.md` as format examples
...
- **Recommendation:** Create a standardized "dsm-docs/ Folder Structure Reference" document in DSM 4.0. For each subfolder, specify: (1) Purpose, (2) Files it contains, (3) Format/template for each file, (4) When files are created/updated. Include a complete reference implementation or link to one. See `backlogs.md`.
...
7. **dsm-docs/ folder structure needs explicit documentation** — AI agents cannot infer folder purposes from names alone. The agent required 6 corrections and external references (sql-agent) to understand where files belong. Folder names are not self-documenting; purpose, contents, format, and relationships must be explicit.
...
- **Finding:** Creating a `done/` subfolder within `dsm-docs/backlog/` for resolved items keeps the backlog actionable. Active items are immediately visible; completed items are preserved for reference.
- **Scores:** Clarity 5, Applicability 5, Completeness 5, Efficiency 5 (Avg: 5.0)
...
- **Context:** Multiple blog posts accumulating in `dsm-docs/blog/<epoch>/` folders. Files named generically (`blog-draft.md`, `linkedin-post.md`) don't sort chronologically.
...
- **Context:** Reviewing `dsm-docs/blog/<epoch>/` folder contents during sprint boundary checklist. Two files exist per epoch: `journal.md` (capture) and `materials.md` (preparation/scoping). The intended distinction: journal is chronological session notes, materials is a structured blog post blueprint.
...
### Entry 20: User-Facing Documentation — No Prescribed Location in dsm-docs/
- **Date:** 2026-02-10 | **Sprint:** Sprint 5 | **Type:** Gap
- **Context:** Sprint 5 produced a remediation guide (`dsm-docs/guides/remediation-guide.md`) and a configuration reference is planned next. Neither artifact fits any existing `dsm-docs/` subfolder.
- **Finding:** DSM 4.0 Section 2 defines `dsm-docs/` subfolders exclusively for project-management artifacts: `checkpoints/`, `decisions/`, `handoffs/`, `feedback/`, `blog/`, `backlog/`, `plan/`. User-facing documentation (installation guides, configuration references, how-tos) has no prescribed location. During Sprint 5, we created `dsm-docs/guides/` ad-hoc to separate user-facing content from PM artifacts. The separation proved immediately useful: the remediation guide is for tool users, not for tracking project progress, and placing it alongside checkpoints or decisions would be confusing.
- **Scores:** Clarity 3, Applicability 4, Completeness 2, Efficiency 4 (Avg: 3.25)
- **Reasoning:** Every existing subfolder has a clear PM purpose. Adding user-facing docs to any of them would blur that purpose. A dedicated `dsm-docs/guides/` subfolder maintains the clean separation DSM already establishes for other artifact types.
- **Recommendation:** Add `dsm-docs/guides/` as a standard subfolder in DSM 4.0 Section 2 for user-facing documentation. See `backlogs.md`.

### Entry 21: Folder Naming Ambiguity — feedback/ vs backlog/
- **Date:** 2026-02-10 | **Sprint:** Sprint 5 | **Type:** Gap
- **Context:** Reviewing `dsm-docs/` folder structure after adding Entry 20. Two subfolders use overlapping terminology: `dsm-docs/feedback-to-dsm/` contains `backlogs.md` (DSM improvement proposals) and `methodology.md` (DSM methodology observations), while `dsm-docs/backlog/` contains cross-project alignment reports.
- **Finding:** The name `feedback/` does not convey directionality. Its contents are specifically feedback *to DSM* (proposals for improving the methodology), but the folder name reads as generic project feedback. Meanwhile, the word "backlog" appears in both contexts with different meanings: `dsm-docs/backlog/` holds alignment reports, `dsm-docs/feedback-to-dsm/backlogs.md` holds DSM improvement proposals. A new contributor or AI agent encountering both folders would struggle to determine which is which without opening files.
...
- **Recommendation:** Rename `dsm-docs/feedback-to-dsm/` to `dsm-docs/feedback-to-dsm-to-dsm/` in DSM 4.0 Section 2. See `backlogs.md`.
...
- **Context:** After logging Entry 23 locally, the question arose of when to push it to DSM Central's inbox. DSM_0.2 says "at session end or sprint boundaries, review dsm-docs/feedback-to-dsm/ for ripe entries." User determined this introduces unnecessary delay.
...
- **Date:** 2026-02-11 | **Sprint:** Sprint 6 | **Type:** Gap
- **Context:** When sending feedback about the session transcript issue, the agent wrote only to DSM Central's inbox, skipping the local feedback files (`dsm-docs/feedback-to-dsm/methodology.md` and `dsm-docs/feedback-to-dsm/backlogs.md`). The user caught the omission.
...
- **Reasoning:** Tests have `tests/`, source code has `src/`, decisions have `dsm-docs/decisions/`, but experiments, despite being a distinct artifact type with their own naming convention (EXP-xxx), lack a designated folder. This creates inconsistency in the project structure template.
...
- **Recommendation:** Add a "Research Gate" to the planning protocol at all scales. Before any plan document is drafted, the agent should assess whether there is unresolved technical uncertainty and, if so, suggest a research phase. The workflow becomes: Idea -> Research (if uncertainty exists) -> Plan -> Action. The research output should be a document in `dsm-docs/research/` that the plan document references. See `backlogs.md` Proposal #27.
- **Pushed:** 2026-03-09
...
- **Context:** During Session 24, the user requested a review of the four-element experiment documentation structure (Justification, Expected Results, Validation, References) used in this project. An online research session validated this structure against six independent frameworks: scientific method, ML tracking tools (MLflow, W&B, Neptune, DVC), Wohlin et al. "Experimentation in Software Engineering," GQM, reproducibility standards (FAIR, NeurIPS checklist, arXiv 2406.14325), and Hypothesis-Driven Development (Cowan, IBM Garage, IEEE 2019). Full research at `dsm-docs/research/experiment-documentation-standards.md`.
...
- **Recommendation:** Adopt the seven-element structure for EXP-006 onwards. Do not backfill EXP-003/004 (decisions already made; retroactive documentation adds little value relative to effort). See `backlogs.md` Proposal #29 and `dsm-docs/research/experiment-documentation-standards.md`.
- **Pushed:** 2026-03-10
...
- **Recommendation:** Add a "Sprint Boundary Gate" step to `/dsm-go` between Step 3.5 (Checkpoint check) and Step 4 (Bandwidth). The gate reads the latest sprint number from MEMORY.md, then verifies boundary artifacts exist: (1) checkpoint in `dsm-docs/checkpoints/done/` matching the sprint, (2) journal entry in the current epoch's blog journal, (3) README status updated for the sprint. If any are missing, report them as blockers and suggest completing the boundary before new work. See `backlogs.md` Proposal #32.
- **Pushed:** 2026-03-12
...
### Entry 42: Open Source Contribution Pipeline — From Experiment to Strategic Visibility
- **Date:** 2026-03-13 | **Sprint:** Post-Epoch 3 | **Type:** Pattern / Strategy
- **Context:** During Session 33, the agent reviewed EXP-005 results and deep-dive research for FalkorDBLite, identifying 5 documentation gaps in the upstream library. Session 34 opened a GitHub issue (FalkorDB/falkordblite#85) documenting these gaps, with a project link back to dsm-graph-explorer. The journal already contained a narrative thread: "when your experiment becomes upstream documentation." This revealed a three-step pattern that converts internal experiment work into external visibility for take-ai-bite and DSM.
- **Finding:** DSM 4.0 capability experiments (Section 4.4) often discover gaps in the libraries they validate: undocumented APIs, missing testing guidance, incomplete examples, version requirements. These gaps are currently documented in internal research files and experiments, benefiting only the project. However, each gap is also a potential open-source contribution, and each contribution is a public artifact that demonstrates take-ai-bite's technical depth and DSM's methodology effectiveness. The pattern is a three-step pipeline: (1) **Issue:** document gaps on the upstream repo, linking back to the take-ai-bite project that discovered them; (2) **Blog post:** narrate the discovery process, showcasing how structured capability experiments (DSM 4.0) systematically surface documentation gaps that ad-hoc usage misses; (3) **PR:** contribute the fix (documentation, examples, tests), establishing take-ai-bite as a contributor in the ecosystem. Each step creates a public, searchable artifact. The issue shows the project exists. The blog post shows the methodology works. The PR shows the team ships. Together, they form a credibility pipeline that compounds across projects and libraries.
- **Scores:** Clarity 5, Applicability 5, Completeness 4, Efficiency 5 (Avg: 4.75)
- **Reasoning:** This pattern requires no additional work beyond what DSM already prescribes: experiments are already run, research is already documented, blog posts are already planned. The only new steps are: (a) assess experiment results for upstream contribution potential, and (b) execute the issue → blog → PR sequence. The strategic value is high: open-source contributions are the most credible form of technical marketing. They are not self-promotional; they solve real problems for real users. The DSM methodology context (structured experiments, tiered research) is the differentiator, as it explains *how* the gaps were found systematically rather than stumbled upon. This pattern scales across all spoke projects: any experiment that validates an external library is a potential contribution pipeline.
- **Recommendation:** Add an "Open Source Contribution Pipeline" pattern to DSM 4.0 or DSM 2.0 (strategic visibility). After completing a capability experiment that validates an external library, the agent assesses whether documentation or usability gaps were discovered. If yes, it suggests the three-step pipeline: (1) GitHub issue with project link, (2) blog post narrating the methodology, (3) PR contributing the fix. The assessment should be a lightweight checkpoint, not a mandatory pipeline: not every gap warrants a contribution. The decision criteria are: (a) is the gap likely to affect other users? (b) do we have enough evidence to document it clearly? (c) does the contribution showcase DSM methodology effectively? See `backlogs.md` Proposal #37.
- **Pushed:** 2026-03-13

### Entry 43: Plan Notification to Hub/Portfolio — Alignment Before Execution
- **Date:** 2026-03-13 | **Sprint:** Epoch 4 planning | **Type:** Gap / Process
- **Context:** After drafting the Epoch 4 plan using DSM Central's alignment response, the user requested: "inform central DSM and ds-portfolio about it and suggest to read it to align with their current work. This pattern should be triggered every time a plan is drafted." The Ecosystem Alignment Gate (Entry 41, Proposal #36) addresses the *inbound* direction: the spoke asks the hub for context before planning. This entry addresses the *outbound* direction: the spoke notifies the hub and portfolio after the plan is drafted, so they can align their own work with the spoke's upcoming direction.
- **Finding:** The planning protocol has an asymmetric notification gap that affects all DSM spoke projects, not just GE. The Ecosystem Alignment Gate (Proposal #36) ensures a spoke consults the hub *before* planning. The Sprint Boundary Notification (Proposal #35) notifies the hub *after* a sprint completes. But between these two points, the drafted plan itself is never communicated. The hub and portfolio learn what a spoke *did* (via sprint completion notifications) but not what it *intends to do* (via plan notifications). In a multi-spoke ecosystem, this creates a window where the hub or other spokes could start work that conflicts with or duplicates a spoke's planned direction. The fix is simple: after drafting a plan, notify hub and portfolio with a summary and a pointer to the full plan, requesting alignment review. This closes the notification loop for any spoke project: alignment request (inbound) → plan draft → plan notification (outbound) → execution → completion notification (outbound).
- **Scores:** Clarity 5, Applicability 5, Completeness 4, Efficiency 5 (Avg: 4.75)
- **Reasoning:** The cost is one `_inbox/` notification per target per plan. The value is proactive alignment: the hub can flag conflicts before any spoke invests sessions in the wrong direction. This is especially valuable when a spoke's plan references hub-level items (like BL-090) where timing and format decisions affect multiple projects. The pattern scales to project size: epoch plans for larger projects, sprint plans for smaller ones. As the DSM ecosystem grows with more spoke projects, coordinating planned work becomes increasingly important; without plan notifications, each spoke plans in isolation and the hub discovers conflicts only after execution.
- **Recommendation:** Add a "Plan Notification" step to the DSM planning protocol (DSM 2.0), applicable to all spoke projects. The step is triggered after any plan is drafted (epoch plan for larger projects, sprint plan for smaller projects). The spoke sends a structured notification to hub and portfolio `_inbox/` folders containing: plan summary, scope table, relevance to recipient's current work, pointer to the full plan, and requested action (review and flag conflicts). This complements the Ecosystem Alignment Gate (Proposal #36, inbound) with an outbound notification, completing the bidirectional alignment loop across the entire DSM ecosystem. See `backlogs.md` Proposal #38.
- **Pushed:** 2026-03-13

### Entry 44: Inbox Protocol Violation — Full File Dump Instead of Structured Notification
- **Date:** 2026-03-13 | **Sprint:** Epoch 4 planning | **Type:** Bug / Protocol Violation
- **Context:** The agent had been copying the full `methodology.md` (76KB) and `backlogs.md` (61KB) files into DSM Central's `_inbox/` folder at every feedback push. The `_inbox/README.md` clearly defines the inbox as a transit point for brief, structured entries following a specific template (Type/Priority/Source + description). The agent was dumping 137KB of full file mirrors into a folder designed for concise notifications. This error compounded across multiple sessions, with each push overwriting the previous copy with an increasingly large file.
- **Finding:** The root cause is an ambiguous memory entry. MEMORY.md states: "Three-file atomic feedback: methodology.md + backlogs.md + DSM Central inbox (always all 3)." The agent interpreted "DSM Central inbox" as the *destination* for the methodology and backlogs files (copy them there), rather than as a *third action*: push a notification entry summarizing what was added. The word "atomic" reinforced the idea of moving all three together as a unit. The correct interpretation is: (1) update methodology.md locally, (2) update backlogs.md locally, (3) push a brief structured notification to DSM Central `_inbox/` summarizing the new entries and pointing to the spoke's files. The `_inbox/README.md` was unambiguous, but the agent relied on the memory shorthand instead of reading the protocol source. This is a case where a memory entry's compression lost critical nuance, and the agent did not verify the compressed understanding against the source.
- **Scores:** Clarity 5, Applicability 5, Completeness 5, Efficiency 3 (Avg: 4.50)
- **Reasoning:** Two failures compounded: (1) the memory entry was ambiguous, and (2) the agent never re-read the `_inbox/README.md` to verify its understanding. The fix has two parts: correct the memory entry to be unambiguous, and add a note to the inbox protocol that agents should never copy full files into `_inbox/`. The DSM_0.2 inbox protocol description should explicitly state the maximum scope of an inbox entry (brief notification with pointer, not file copy). This applies to all DSM spoke projects, as any agent reading the same compressed memory could make the same mistake.
- **Recommendation:** (1) Correct the MEMORY.md entry to: "Three-file atomic feedback: update methodology.md locally, update backlogs.md locally, push a brief structured notification to DSM Central `_inbox/dsm-graph-explorer.md` (never copy full files)." (2) Add to `_inbox/README.md`: "Inbox entries are notifications, not file mirrors. Never copy full feedback files into `_inbox/`. Reference the spoke's files by path instead." See `backlogs.md` Proposal #39.
- **Pushed:** 2026-03-13

### Entry 45: Inbox Filename Convention — Date and Content Identification
- **Date:** 2026-03-13 | **Sprint:** Epoch 4 planning | **Type:** Gap / Protocol
- **Context:** During Session 34, after correcting the full-file-dump error (Entry 44), the agent created the inbox notification file as `dsm-graph-explorer.md` (no date, no content identifier). The `_inbox/README.md` prescribes "One file per spoke project: `{project-name}.md`," which the agent followed. However, this convention loses temporal context: when was the notification pushed? What entries does it cover? The user corrected this, requesting the filename include the date and the entry range. The correct naming convention is: `{date}_{project-name}_{content-description}.md`, e.g., `2026-03-13_dsm-graph-explorer_feedback-entries-42-44.md`.
- **Finding:** The `_inbox/README.md` filename convention (`{project-name}.md`) was designed for a single-file-per-spoke model where entries accumulate and get cleared after processing. This model has two problems: (1) no temporal context, as the filename doesn't indicate when notifications arrived, making it impossible to prioritize by recency; (2) no content identification, as the hub must open the file to understand what it contains. A dated, content-identified filename is self-documenting: the hub can triage inbox entries by scanning filenames alone. For feedback notifications specifically, the convention should be `{date}_{project-name}_feedback-entries-{from}-{to}.md`, making it immediately clear which entries are covered.
- **Scores:** Clarity 5, Applicability 5, Completeness 4, Efficiency 5 (Avg: 4.75)
- **Reasoning:** Dated filenames are a near-universal convention in structured communication systems (email subjects include dates, log files include timestamps, git commits include dates). The `_inbox/` is an asynchronous communication channel; without dates in filenames, the only temporal signal is the filesystem's modification timestamp, which is fragile (copies, moves, edits all change it). The content description prevents the hub from needing to open every file to decide what to process first.
